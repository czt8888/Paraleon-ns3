...
void RdmaHw::Setup(QpCompleteCallback cb){
	for (uint32_t i = 0; i < m_nic.size(); i++){
		...
		m_nic_datarate_path = "mix/nic_datarate.tr";
		m_nic_cnp_received_path = "mix/nic_cnp_received.tr";
...
void RdmaHw::ChangeParameters(std::map<std::string, std::string> parameter_map)
{
	for (auto it : parameter_map)
	{
		...
		if (parameter_key_str == "rate_to_set_on_first_cnp" || parameter_key_str == "rpg_min_dec_fac" || parameter_key_str == "time_reset" || parameter_key_str == "rateDecreaseInterval")
		{
