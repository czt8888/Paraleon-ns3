...
public:
    ...
    {
        uint32_t swap_key[KEY_LENGTH_4];
        uint32_t swap_val = 0;
        //printf("insert!!\n");
    ...
    void quick_insert(uint32_t *key, int f = 1)
    {
    ...
    void get_heavy_hitters(int threshold, vector<pair<uint32_t, int>> & results)
    {
        for (int i = 0; i < bucket_num; ++i) 
            for (int j = 0; j < MAX_VALID_COUNTER; ++j) 
            {
                ...
                uint32_t val = query((uint32_t *)&key);
                // printf("key=%lu, value=%d\n", key, val);
                if (val >= threshold) {
                    // results.push_back(make_pair(string((const char*)&key, 32), val));
...
/* interface */
    ...
    void get_lightpart_distribution(vector<double> &dist)
    {
        light_part.get_distribution(dist);
    }
